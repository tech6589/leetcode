 ...
 * 
 */
// return max{distance of nearest heater for each house}
// x a   b:     2x<a+b
// a x   b:     2x<a+b
// a  x  b:     2x==a+b
// a   x b:     2x>a+b
...
class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for(int i=0; i<houses.length; i++){
            for(; j+1<heaters.length && 2*houses[i]>heaters[j]+heaters[j+1]; j++){
            }
            // if 2*houses[i] is always bigger than heaters[j]+heaters[j+1], then j would be heaters.length-1, which is the last heater
